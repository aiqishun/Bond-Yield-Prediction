...
| `lpr.xlsx` | `order_date_dt`，`year_1_lpr`，`year_5_lpr` | 月频 | LPR 因子（低频，转日频后 forward fill；主用途固定：直接因子） |
| `债券发行与到期20250818国债及政金债.xlsx` | （当前仓库内字段与 `lpr.xlsx` 相同） | 月频 | 旧数据包遗留文件，pipeline 默认不使用 |
...
| `lpr_notes.txt` | - | - | 仅为说明/备注文本，不参与 pipeline |
...
- `data/lpr_notes.txt` 是说明文本，不参与合并；LPR 因子来源为 `data/lpr.xlsx`
